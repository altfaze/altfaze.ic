...
export default function FreelancerRequestsPage() {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      const res = await fetch(`/api/requests?type=received&page=${page}&limit=${limit}`, {
        cache: 'no-store',
      })
    ...
    <div className="min-h-screen bg-background">
      <div className="container py-12 max-w-4xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Manage project requests from clients</p>
        </div>
        ...
        {requests.length > 0 ? (
          ...
          <Card>
            <CardContent className="pt-12 pb-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Update your profile to start receiving requests
              </p>
            </CardContent>
          </Card>
        )}
