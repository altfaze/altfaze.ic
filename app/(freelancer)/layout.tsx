...
export default async function FreelancerLayout({ 
  ...
  try {
    ...
    if (!user.role) {
      console.warn('[FREELANCER_LAYOUT] ⚠️ User has no role set - checking database...')
      // Double-check database to ensure role wasn't just set
      if (user.email) {
        ...
        if (!dbUser?.role) {
          console.warn('[FREELANCER_LAYOUT] ⚠️ Database also shows no role - redirecting to onboard for role selection')
          return redirect('/onboard')
        }
        console.log('[FREELANCER_LAYOUT] ✅ Role found in database:', dbUser.role)
        user.role = dbUser.role
      } else {
        console.warn('[FREELANCER_LAYOUT] ⚠️ No email in user - redirecting to onboard')
        return redirect('/onboard')
      }
    }
